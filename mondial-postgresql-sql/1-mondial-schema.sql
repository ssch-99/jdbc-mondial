...
(Name VARCHAR(50) NOT NULL UNIQUE,
 ...
 Capital VARCHAR(50),
 Province VARCHAR(50),
 Area real CONSTRAINT CountryArea
   CHECK (Area >= 0),
...
(Name VARCHAR(50),
 ...
 Population integer CONSTRAINT CityPop
   CHECK (Population >= 0),
 Latitude real CONSTRAINT CityLat
   CHECK ((Latitude >= -90) AND (Latitude <= 90)) ,
 Longitude real CONSTRAINT CityLon
...
(Name VARCHAR(50) CONSTRAINT PrName NOT NULL ,
 ...
 Population integer CONSTRAINT PrPop
   CHECK (Population >= 0),
 Area real CONSTRAINT PrAr
   CHECK (Area >= 0),
 Capital VARCHAR(50),
 CapProv VARCHAR(50),
...
(Country VARCHAR(4) CONSTRAINT EconomyKey PRIMARY KEY,
 GDP real CONSTRAINT EconomyGDP
   CHECK (GDP >= 0),
 Agriculture real,
 Service real,
 Industry real,
...
(Country VARCHAR(4),
 Name VARCHAR(50),
 Percentage real CONSTRAINT LanguagePercent 
...
(Country VARCHAR(4),
 Name VARCHAR(50),
 Percentage real CONSTRAINT ReligionPercent 
   CHECK ((Percentage > 0) AND (Percentage <= 100)),
...
(Country VARCHAR(4),
 Name VARCHAR(50),
 Percentage real CONSTRAINT EthnicPercent 
   CHECK ((Percentage > 0) AND (Percentage <= 100)),
 CONSTRAINT EthnicKey PRIMARY KEY (Name, Country));

CREATE TABLE Countrypops
(Country VARCHAR(4),
 Year integer CONSTRAINT CountryPopsYear
   CHECK (Year >= 0),
 Population integer CONSTRAINT CountryPopsPop
   CHECK (Population >= 0),
...
(Province VARCHAR(50),
 Country VARCHAR(4),
 Year integer CONSTRAINT ProvPopsYear
   CHECK (Year >= 0),
 Population integer CONSTRAINT ProvPopsPop
   CHECK (Population >= 0),
...
(City VARCHAR(50),
 Country VARCHAR(4),
 Province VARCHAR(50),
 Year integer CONSTRAINT CityPopsYear
   CHECK (Year >= 0),
 Population integer CONSTRAINT CityPopsPop
   CHECK (Population >= 0),
...
(Name VARCHAR(50) CONSTRAINT IslandKey PRIMARY KEY,
 Islands VARCHAR(50),
 Area real CONSTRAINT IslandAr check (Area >= 0),
 Elevation real,
 Type VARCHAR(10),
...
(Name VARCHAR(50) CONSTRAINT LakeKey PRIMARY KEY,
 Area real CONSTRAINT LakeAr CHECK (Area >= 0),
 Depth real CONSTRAINT LakeDpth CHECK (Depth >= 0),
 Elevation real,
 Type VARCHAR(12),
 River VARCHAR(50),
...
(Name VARCHAR(50) CONSTRAINT SeaKey PRIMARY KEY,
 Depth real CONSTRAINT SeaDepth CHECK (Depth >= 0));
...
(Name VARCHAR(50) CONSTRAINT RiverKey PRIMARY KEY,
 ...
 Lake VARCHAR(50),
 Sea VARCHAR(50),
 Length real CONSTRAINT RiverLength
   CHECK (Length >= 0),
 Area real CONSTRAINT RiverArea
   CHECK (Area >= 0),
...
(IATACode VARCHAR(3) PRIMARY KEY,
 ...
 Province VARCHAR(50) ,
 Island VARCHAR(50) ,
 Latitude real CONSTRAINT AirpLat
   CHECK ((Latitude >= -90) AND (Latitude <= 90)) ,
 Longitude real CONSTRAINT AirpLon
   CHECK ((Longitude >= -180) AND (Longitude <= 180)) ,
 Elevation real ,
 gmtOffset real );
